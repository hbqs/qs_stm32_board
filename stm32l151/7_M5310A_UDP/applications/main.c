...
udp_recon:
	//udp client 初始化
	udp_client=qsdk_net_client_init(QSDK_NET_TYPE_UDP,1003,"114.116.144.151",2039);
	
	//udp client 创建 socket
	if(qsdk_net_create_socket(udp_client)!=RT_EOK)
		...
		if(rt_event_recv(net_error,EVENT_SEND_ERROR|EVENT_REBOOT,RT_EVENT_FLAG_OR|RT_EVENT_FLAG_CLEAR,RT_WAITING_FOREVER,&event_status)==RT_EOK)
		{
			...
			if(event_status==EVENT_SEND_ERROR)
			{
				...
				if(qsdk_nb_get_net_connect()!=RT_EOK)
				{
					...
					recon_count=1;
					qsdk_net_close_socket(udp_client);
					goto net_recon;
				}
				LOG_E("Now udp will reconnection\r\n");
				qsdk_net_close_socket(udp_client);
			...
			{
				if(qsdk_net_get_client_connect(udp_client)!=RT_EOK)
				{
...
void data_up_entry(void *parameter)
{
	...
	while(1)
	{
		...
		if(qsdk_nb_get_net_connect_status()==RT_EOK)
		{
			...
			if(qsdk_net_get_client_connect(udp_client)==RT_EOK)
			{
				LOG_I("data UP is open\r\n");
data_repeat:				
				if(qsdk_net_send_data(udp_client,buf)==RT_EOK)
						LOG_I("udp send data success\r\n");
				else
				{
					LOG_E("udp send data failure\r\n");
